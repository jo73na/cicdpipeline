...
app.use("/leads", upload.single('leadFile'), authAdmin, async (req, res) => {
  ...
  let success = jsonData.map(async (item) => {
      ...
      if (normalizedAccountName !== "" && !processedCompanies.includes(normalizedAccountName)) {
          ...
          const existingAccount = await crud.getOneDocument(accounts, { account_name: normalizedAccountName }, {}, {});
          let accountId;
          ...
          if (existingAccount) {
              ...
              const addedData = {
                  ...
                  linkedin_url: item.CompanyLinkedinUrl,
          ...
          const contactData = {
              ...
              followup_1: item.FollowUp1,
              followup_2: item.FollowUp2
          };
