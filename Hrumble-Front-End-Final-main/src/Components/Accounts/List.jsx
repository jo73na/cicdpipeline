...
const Lists=()=>{
  ...
  const {fetchList,Listdata,paginationcontact,setPaginationcontact,contactLinkDrawer,setcontactlinkDrawer,handleViewContact,ViewContactDrawer,ViewContactPopup,AddContactPopup,AddContactDrawer,contacts,AddCompanyPopup,Loading,AddCompanyDrawer,fethContacts,}=useContext(SalesandMargettingContext)
  const navigate=useNavigate()
...
//  Pie Chart ends here
 ...
 let mapedData =Listdata?.map((item, index)=>{
     ...
     <td>
                                                <div className="products">
                                                    ...
                                                    <div>
                                                        <h6><Link to={`/listcontacts/${item?._id}`}>{item.name}</Link></h6>
     ...
     <td><span>{item?.no_of_contacts || "-"}</span></td>
   
     <td><span>{item?.created_by?.name||"-"}</span></td>
     <td><span>{dayjs(item?.updatedAt).format("YYYY-MM-DD")||"-"}</span></td>
				...
				<div className="row">
                    ...
                    <Tab.Container defaultActiveKey={'Grid'} >
                        <div className="d-flex justify-content-between align-items-center mb-4">
                            <h4 className="heading mb-0">Saved List</h4>
