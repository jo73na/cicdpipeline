...
// import ClientContext from '../../../context/CleintContext';
...
const Clients = () => {	
    ...
    const {fetchClient,clients,Loading,openDrawer,handleopenDrawer,editDrawer,handleOpenEditDrawer,handleChangeSearchClient} = useContext(ClientContext);
    ...
    clients?.forEach((currentValue) => {
      totalprojects += Number(currentValue.totalprojects || 0);
      Ongoingprojects += Number(currentValue.ongoing || 0);
      Completedprojects += Number(currentValue.completed || 0);
      revenue += Number(currentValue.revenue || 0);
      profit += Number(currentValue.salary || 0);
    });
                                    ...
                                    <tbody>
                                        {clients?.map((client, ind)=>(
                                            <tr>
                                                ...
                                                <td> {`${client?.revenue?.toLocaleString('en-IN', {
        style: 'currency',
        currency: 'INR',
      })}`}</td>
                                                <td>{ client?.revenue&&client?.salary?`${(client?.revenue-client?.salary||"-")?.toLocaleString('en-IN', {
        style: 'currency',
        currency: 'INR',
      })}`:"-"}</td>
                                                {/* <td>
                                                ...
                                                {/* <td>
                                                    <Dropdown>
														...
														<Dropdown.Item>View</Dropdown.Item>
                                                            ...
                                                            <Dropdown.Item
															 onClick={(e)=>handleOpenEditDrawer(client?._id)}>Edit</Dropdown.Item>
...
width={900}
height={300}
open={openDrawer}  onCancel={handleopenDrawer}
