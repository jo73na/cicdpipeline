import { Space, Cascader, Row, Col, Select, Input, Button, message } from "antd";
...
import { MinusCircleOutlined, PlusOutlined, DollarOutlined } from "@ant-design/icons";
...
const options = [{
    ...
    const menuThree = (parent, menu) => {
      ...
      const filter = menu?.filter((e) => String(e?.parent?._id || null) === String(parent));
...
// Step 4 Validation
  ...
  useEffect(() => {
    if (job_type === "Full Time") { // Full Time
      setSalaryType("LPA");
    ...
    } else if (job_type === "Contract") { // Contract
...
<Input
 addonAfter={
   job_type === "Full Time" ? (
     ...
     >
       {(job_type === "Both" || job_type === "Full Time") && (
         <Option value="LPA">LPA</Option>
       )}
       {(job_type === "Both" || job_type === "Contract") && (
         <>
...
>
  ...
  />
          ...
          <Row gutter={[16, 16]}>
            ...
            <Form.Item
                    ...
                    rules={[
                      { required: false, message: "Please input the Point of Contact!" },
                    ]}
                  >
  ...
  return (
    <div className="hiring-form-container">
      ...
      >
        <Stepper activeStep={activeStep} orientation="vertical" className={useStyles().root}>
