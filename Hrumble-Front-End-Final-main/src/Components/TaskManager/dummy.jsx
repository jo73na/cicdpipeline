...
const recordsPerPage = 30;
    ...
    // Update records whenever tableData or pagination changes
    useEffect(() => {
        const lastIndex = currentPage * recordsPerPage;
        const firstIndex = lastIndex - recordsPerPage;
        
        // Ensure we don't exceed array bounds
        const slicedData = tableData.slice(firstIndex, Math.min(lastIndex, tableData.length));
        setRecords(slicedData);
        
        // If current page is greater than total pages, set to last available page
    ...
    // Update the pagination display component
    const PaginationDisplay = () => {
        return (
            <div className="d-sm-flex text-center justify-content-between align-items-center mt-3">
                <div className='dataTables_info'>
                    Showing {firstIndex + 1} to{" "}
                    {Math.min(lastIndex, tableData.length)} of {tableData.length} entries
                </div>
