...
const InvoiceMonth = ({year}) => {

  const { FetchInvoice, employeeLogindata , employeeFull, fetchEmployFull, setFileOpen} = useContext(FileManagerContext);
  ...
  const handleEmployeeDoc = (year,index) => {
    FetchInvoice(year,index)// Handle document for employee with the given id
    setFileOpen(true);
  };
