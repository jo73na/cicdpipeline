...
const {Search}=Input
const Invoice = () => {
    const {setedit,invoice,Loading,handleviewInvoice,handleClosepaymnet,recordopen} =useContext(InvoiceExpenceContext)
...
const checkstatusSelect = (dueDate,status,id) => {
    const today = new Date();
    const due = new Date(dueDate);
    const differenceInDays = Math.floor((today - due) / (1000 * 60 * 60 * 24));
   
    if (differenceInDays>0 ){
    return<span className={`badge badge-${checkstatus(dueDate,status)} light border-0 me-1`}>
     {`Overdue by ${differenceInDays} day`}
...
function formatIndianCurrency(amount) {
  ...
  let mapedData =invoice?.map((item, index)=>{
     ...
     <td><span>₹{formatIndianCurrency(item?.total_amount)}</span></td>
     <td><span> {item?.paid_amount?  formatIndianCurrency(item?.paid_amount):"-"}</span></td>
     <td><span>{dayjs(item?.due_date).format(" DD-MM-YYYY")}</span></td>
     <td  style ={{
         textAlign:"left"
     }}>                                                     {checkstatusSelect(item.due_date,item.status,item?._id)}
  ...
  return (
    <>  
    {
       Loading ?
        ...
        width={800}
      >
        <Invoiceinfo setDrawerInvoice={setDrawerInvoice}/>
